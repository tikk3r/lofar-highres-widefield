...
tok_type = 'ELAIS_' + obsid
jsonfile = '/project/lofarvwf/Share/ELAIS-N1/long_baseline_di/lb_step1_sub6asec.json'
...
g = s.sbn_dict(pref='[a-zA-Z0-9]{10}_', suff='MHz')
...
for k,v in d.items():
    #tok = caToken(database = db, token_type = tok_type, token_id = tok_type + '_cal_' + str(cal_obsid) + '_SB' + str(k))
    tok = caToken(database = db, token_type = tok_type, token_id = tok_type + '_sub6asec_' + str(cal_obsid) + '_' + str(k) + 'MHz')
...
for tok in tl:
    ...
    tok['PIPELINE_STEP'] = 'lb_sub6asec_targ1'
    #tok['PIPELINE_STEP'] = 'lb_image_1asec_cal1'
    tok['status'] = 'queued'
    tok.save()

tl.add_attachment(attachment_name = 'step1_subtract_lotss.parset', filename = '/project/lofarvwf/Share/ELAIS-N1/long_baseline_di/step1_subtract_lotss.parset')
...
view_cal = TokenView('step1_subtract_lotss', condition='doc.PIPELINE_STEP=="lb_sub6asec_targ1"', emit_values=('doc._id', 'doc.status'))
tl.add_token_views()
...
#j = submit.SpiderLauncher(numjobs=len(d.keys()), token_type=tok_type, wholenode=True, parameter_step=24, NCPU=24)
j = submit.SpiderLauncher(numjobs=len(d.keys()), token_type=tok_type, wholenode=False, parameter_step=24, NCPU=40)
