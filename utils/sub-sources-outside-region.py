...
def fixsymlinks(ddsols):
    ...
    for i in range(0,len(dds3)):
        symsolname = dds3[i].split('killMS.' + ddsols + '.sols.npz')[0] + 'killMS.'+ddsols+'_smoothed.sols.npz' #dds3smoothed[i]
        solname = dds3[i]
        #ddsols = 'DDS3_full'
...
def filechecker(clustercat, dicomask, indico, h5sols, HMPmodelfits):
  '''
...
args = vars(parser.parse_args())
    ...
    #print args['DDESolutions_DDSols']
#else: 
    #print args['DDESolutions_DDSols']    
#sys.exit()
...
filechecker(clustercat, fullmask, indico, args['h5sols'], args['HMPmodelfits'])
if args['h5sols'] == None:
  if args['DDESolutions_DDSols']:
